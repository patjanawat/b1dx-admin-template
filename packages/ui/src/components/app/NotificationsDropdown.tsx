...
export const NotificationsDropdown: React.FC<AppNotificationsDropdownProps> = ({
  ...
  emptyStateLabel = 'No new notifications',
  emptyStateHint = 'You are all caught up.'
}) => {
  const computedNewCount = newCount ?? items.filter((item) => item.unread).length;
        ...
        <div className="flex items-center gap-3">
          ...
          {computedNewCount > 0 && (
            <span className="px-2 py-0.5 bg-indigo-100 text-indigo-700 text-[10px] font-bold rounded-md uppercase tracking-wider dark:bg-indigo-900/40 dark:text-indigo-300">
              {computedNewCount} {newLabel}
            </span>
          )}
        </div>
        <button
          className="text-slate-400 hover:text-indigo-600 transition-colors p-1 rounded-md hover:bg-white dark:hover:bg-slate-800"
          ...
          items.map((item) => (
            ...
            >
              <div className="flex-shrink-0">
                {item.type === 'image' && item.avatarUrl && (
                  <img src={item.avatarUrl} alt={item.title} className="w-10 h-10 rounded-full object-cover shadow-sm" />
                )}
                {item.type === 'initials' && (
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-xs shadow-sm dark:shadow-none ${
                      item.initialsClassName ?? 'bg-slate-100 text-slate-600'
                    } dark:opacity-80`}
                  >
                    {item.initials}
                  </div>
                ...
                {item.type === 'icon' && (
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center shadow-sm dark:shadow-none ${
                      item.iconBgClassName ?? 'bg-slate-100'
                    } ${item.iconColorClassName ?? 'text-slate-600'} dark:opacity-80`}
                  >
                    {item.icon}
                  </div>
                )}
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-start justify-between gap-2">
                  <p className="text-sm font-bold text-slate-800 truncate dark:text-slate-200">{item.title}</p>
                  {item.unread && (
