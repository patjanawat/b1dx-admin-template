    ...
    <div className="min-h-screen bg-background flex flex-col items-center justify-center p-6 font-['Inter'] transition-colors duration-200">
      ...
      <div className="w-full max-w-[440px] bg-white dark:bg-slate-900 rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 dark:border-slate-800 p-10 md:p-12 text-center transition-colors">
        ...
        <form onSubmit={handleSubmit} className="space-y-6 text-left">
          {/* New Password Field */}
          <div>
            <label className="block text-sm font-semibold text-slate-600 dark:text-slate-400 mb-2.5">New Password</label>
            <div className="relative group">
              <input
                ...
                placeholder="........"
                className="w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl py-3.5 px-4 pr-12 text-sm focus:ring-4 focus:ring-blue-500/5 focus:border-[#1d8cf8] transition-all outline-none text-slate-900 dark:text-white placeholder:text-slate-300 dark:placeholder:text-slate-600"
                required
              />
              <button
                type="button"
          ...
          <div>
            <label className="block text-sm font-semibold text-slate-600 dark:text-slate-400 mb-2.5">Confirm New Password</label>
            <div className="relative group">
              <input
          ...
          {/* Security Checklist */}
          <div className="bg-slate-50 dark:bg-slate-800/50 rounded-xl p-5 mt-2 transition-colors">
            <h3 className="text-[11px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.1em] mb-3">Security Checklist</h3>
            <div className="space-y-2.5">
              <div className="flex items-center gap-2.5 text-[13px] font-medium transition-colors">
                {isMinLength ? (
                  ...
                  <Circle size={14} className="text-slate-300 dark:text-slate-700 fill-slate-300 dark:fill-slate-700" />
                )}
                <span className={isMinLength ? 'text-slate-600 dark:text-slate-300' : 'text-slate-500 dark:text-slate-500'}>At least 8 characters long</span>
              </div>
              <div className="flex items-center gap-2.5 text-[13px] font-medium transition-colors">
                {hasUpperAndNumber ? (
                  <Check size={14} className="text-emerald-500 stroke-[3px]" />
                ...
                <span className={hasUpperAndNumber ? 'text-slate-600 dark:text-slate-300' : 'text-slate-500 dark:text-slate-500'}>One uppercase and one number</span>
              </div>
            </div>
          </div>
          ...
          <button
            type="submit"
            className="w-full bg-[#1d8cf8] hover:bg-[#1a7cdb] text-white font-bold py-4 rounded-xl shadow-lg shadow-blue-100 dark:shadow-none flex items-center justify-center gap-2 transition-all active:scale-[0.98] mt-4"
          >
            Reset Password
          </button>
        </form>
