          ...
          {isThemeOpen && (
            <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-2xl border border-slate-200 py-1.5 z-50 animate-dropdown dark:bg-slate-800 dark:border-slate-700">
              {themes.map((theme) => (
                ...
                >
                  <theme.icon size={18} className={activeTheme === theme.id ? 'text-indigo-500 dark:text-indigo-400' : 'text-slate-400'} />
                  {theme.label}
                </button>
              ))}
            </div>
        ...
        <div className="relative" ref={profileDropdownRef}>
          <div 
            ...
            className={`flex items-center gap-3 cursor-pointer group p-1 pr-2 rounded-xl transition-all duration-200 active:scale-95 ${
              isProfileOpen ? 'bg-slate-100 ring-1 ring-slate-200 dark:bg-slate-800 dark:ring-slate-700' : 'hover:bg-slate-50 dark:hover:bg-slate-800'
            }`}
          >
            <div className="relative">
              ...
              />
              <span className="absolute bottom-0 right-0 w-2.5 h-2.5 bg-emerald-500 border-2 border-white rounded-full dark:border-slate-800"></span>
            </div>
            <div className="text-left hidden sm:block">
              <p className="text-sm font-bold text-slate-800 leading-none dark:text-slate-200">Alex Rivera</p>
              <p className="text-[11px] text-slate-500 font-medium mt-1 uppercase tracking-tighter dark:text-slate-400">Warehouse Lead</p>
            </div>
          </div>
