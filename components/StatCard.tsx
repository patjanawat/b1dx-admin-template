      ...
      <CardContent className="p-6">
        <div className="flex items-center justify-between gap-4">
          <div className="flex-1 min-w-0">
            <p className="text-sm font-medium text-slate-500 tracking-tight truncate">
              {label}
            </p>
            ...
            <div className={`mt-2.5 flex items-center gap-1.5 font-medium text-xs ${getFooterColor()}`}>
              ...
              <span className={`${change ? 'text-slate-400 font-normal' : ''} truncate`}>
                {footerText}
              </span>
            </div>
          </div>
          
          <div className={`${iconBg} ${iconColor} w-12 h-12 flex items-center justify-center rounded-xl shrink-0 shadow-sm border border-slate-100/50`}>
            {icon}
          </div>
        </div>
      </CardContent>
