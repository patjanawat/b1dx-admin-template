  ...
  return (
    <div className="w-[360px] bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden dark:bg-slate-900 dark:border-slate-800">
      ...
      <div className="px-5 py-4 flex items-center justify-between border-b border-slate-100 bg-slate-50/30 dark:bg-slate-800/50 dark:border-slate-800">
        <h3 className="font-bold text-slate-800 dark:text-slate-100">Shortcuts</h3>
        <button className="p-1.5 hover:bg-white rounded-lg transition-colors text-slate-500 hover:text-indigo-600 border border-transparent hover:border-slate-200 dark:hover:bg-slate-800 dark:hover:border-slate-700 dark:text-slate-400">
          <Plus size={18} />
        </button>
      </div>
        ...
        {SHORTCUTS.map((item, index) => (
          <div 
            ...
            className={`
              ...
              ${index < SHORTCUTS.length - 2 ? 'border-b border-slate-50 dark:border-slate-800' : ''}
            `}
          >
            {/* Icon Wrapper */}
